package org.talangsoft.algorithms.week6;

import org.hamcrest.Matchers;
import org.junit.Assert;
import org.junit.Test;
import org.talangsoft.algorithms.helper.InputFileReader;

import java.util.Optional;

public class MedianMaintenanceTest {

    private String sourceFile = "week6/median.txt";


    @Test
    public void insert5ElementsTest() throws Exception{
        Median m = new Median();
        Assert.assertThat(m.median(), Matchers.is(Optional.empty()));
        m.insert(1);
        Assert.assertThat(m.median(), Matchers.is(Optional.of(1)));
        m.insert(5);
        Assert.assertThat(m.median(), Matchers.is(Optional.of(1)));
        m.insert(4);
        Assert.assertThat(m.median(), Matchers.is(Optional.of(4)));
        m.insert(2);
        Assert.assertThat(m.median(), Matchers.is(Optional.of(2)));
        m.insert(3);
        Assert.assertThat(m.median(), Matchers.is(Optional.of(3)));
    }

    @Test
    public void solveWeek6AssignmentTest() throws Exception{
        int[] numbers = InputFileReader.readIntElements(sourceFile);
        Assert.assertThat(numbers.length, Matchers.is(10000));

        Median m = new Median();
        int sumMedian = 0;
        for(int nr : numbers){
            m.insert(nr);
            sumMedian += m.median().get();
        }
        int solution = sumMedian % 10000;
        Assert.assertThat(solution, Matchers.is(1213));
    }
}
